...
# === IB Gateway Auto-Deploy (stub kept) ===
...
if [ -f "$IBKR_INSTALLER_PATH" ]; then
  ...
  if [ -f "$IBKR_SYSTEMD_UNIT" ]; then
    ...
    echo "$LOG_TAG Linked ibgateway.service (user)."
  fi
else
...
# Kick off today’s one-shot supervisor (safe no-op if locks present)
TODAY_UTC="$(date -u +%F)"
...
echo "$LOG_TAG Done. Web UI is active; supervisor is scheduled and started for today if permitted by locks."
