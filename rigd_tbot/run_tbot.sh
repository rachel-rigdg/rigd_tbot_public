...
# Best-effort: ensure user manager exists and we can talk to it
...
ln -sf "$SYSTEMD_UNIT_PATH/tbot_bot.service" "$HOME/.config/systemd/user/"
...
if [ -f "$IBKR_INSTALLER_PATH" ]; then
  ...
  if [ -f "$IBKR_SYSTEMD_UNIT" ]; then
    ...
    echo "$LOG_TAG [WARN] ibgateway.service unit not found; stub only."
  fi
else
...
# Try normal path; if DBus missing, try machine scope as fallback
if ! systemctl --user daemon-reload 2>/dev/null; then
  echo "$LOG_TAG [WARN] DBus not available; attempting machine-scoped userctl"
  systemctl --user --machine="$(whoami)@.host" daemon-reload
fi

echo "$LOG_TAG Enabling and starting Web UI (tbot_bot.service)…"
if ! systemctl --user enable --now tbot_bot.service 2>/dev/null; then
  systemctl --user --machine="$(whoami)@.host" enable --now tbot_bot.service
fi

echo "$LOG_TAG Enabling daily supervisor timer (UTC 00:01 on trading days)…"
if ! systemctl --user enable --now tbot_supervisor.timer 2>/dev/null; then
  echo "$LOG_TAG [WARN] Failed to enable/start tbot_supervisor.timer (trying machine scope)…"
  systemctl --user --machine="$(whoami)@.host" enable --now tbot_supervisor.timer || echo "$LOG_TAG [WARN] Timer enable still failed."
fi

# Fire today’s one-shot run (safe no-op if locks exist or already ran)
TODAY_UTC="$(date -u +%F)"
echo "$LOG_TAG Starting today's supervisor instance: tbot_supervisor@${TODAY_UTC}.service"
if ! systemctl --user start "tbot_supervisor@${TODAY_UTC}.service" 2>/dev/null; then
  systemctl --user --machine="$(whoami)@.host" start "tbot_supervisor@${TODAY_UTC}.service" || true
fi

echo "$LOG_TAG Done. Web UI is up; supervisor is scheduled daily and triggered for today (subject to lock checks)."
