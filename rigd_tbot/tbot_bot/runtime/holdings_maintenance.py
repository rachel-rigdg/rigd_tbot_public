...
def main() -> int:
    ...
    lock_path = Path(get_holdings_lock_path(trading_date))

    # If we've already run today, exit cleanly (idempotent behavior).
    if lock_path.exists():
        msg = f"Lock exists ({lock_path.name}); holdings maintenance already ran for {trading_date}."
        log.info(msg)
        _write_local_log(msg)
        return 0

    if not _is_bot_ready():
        msg = "Bot not ready (bootstrap or initializing); skipping holdings maintenance."
        log.info(msg)
        _write_local_log(msg)
        return 0
    ...
    try:
        log.info("Starting holdings maintenance cycle (single-run).")
        _write_local_log("Starting holdings maintenance cycle.")
        perform_holdings_cycle()
        # Success → stamp lock
        lock_path.parent.mkdir(parents=True, exist_ok=True)
        lock_path.write_text(datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ") + "\n", encoding="utf-8")
        msg = f"Holdings maintenance completed; lock written: {lock_path.name}"
        log.info(msg)
        _write_local_log(msg)
        return 0
    except Exception as e:
        # Failure → do NOT create lock; allow external retry policy if desired
        err = f"Exception in holdings_maintenance: {e}"
        log.error(err)
