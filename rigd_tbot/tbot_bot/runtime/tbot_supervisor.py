...
# DO NOT resolve/launch strategy_router as a process.
...
def launch_subprocess(cmd_path):
    print(f"[tbot_supervisor] launching process: {os.path.basename(str(cmd_path))}", flush=True)
...
def main():
    ...
    for name, path in launch_targets:
        ...
        if not ensure_singleton(script_name):
            print(f"[tbot_supervisor] Launching {script_name}...", flush=True)
            processes[name] = launch_subprocess(path)
        else:
            print(f"[tbot_supervisor] {script_name} already running.", flush=True)
    ...
    try:
        while True:
            ...
            if state not in BOOT_PHASES:
                ...
                if is_time_for_broker_sync():
                    if not ensure_singleton("sync_broker_ledger.py"):
                        print("[tbot_supervisor] Launching nightly broker sync (sync_broker_ledger.py)...", flush=True)
                ...
                if is_time_for_ledger_snapshot():
                    if not ensure_singleton("ledger_snapshot.py"):
                        print("[tbot_supervisor] Launching nightly EOD ledger snapshot (ledger_snapshot.py)...", flush=True)
            ...
            if TEST_MODE_FLAG.exists():
                print("[tbot_supervisor] Global TEST_MODE flag detected. Launching integration_test_runner.py...", flush=True)
            ...
            if is_time_for_universe_rebuild():
                ...
                if not ensure_singleton("universe_orchestrator.py"):
                    print("[tbot_supervisor] Triggering universe cache rebuild (universe_orchestrator.py)...", flush=True)
            ...
            for name, proc in list(processes.items()):
                if proc.poll() is not None:
                    if name in NON_RESTARTABLE:
                        ...
                        for t_name, t_path in (launch_targets + persistent_ops):
                            if t_name == name:
