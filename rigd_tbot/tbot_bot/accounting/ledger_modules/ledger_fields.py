...
Notes
- OFX-aligned identifier is `fitid` (TEXT UNIQUE).
- All timestamps are UTC ISO-8601 strings (`*_utc`).
...
TRADES_FIELDS: List[str] = [
    ...
    # --- business semantics ---
    "action",                 # TEXT NOT NULL (long/short/put/call/assignment/exercise/expire/reorg/inverse/other)
    "side",                   # TEXT NOT NULL (debit|credit)
    "symbol",                 # TEXT
    "quantity",               # REAL/NUMERIC
    "price",                  # REAL/NUMERIC
    "total_value",            # REAL/NUMERIC NOT NULL (sign by side)
    "amount",                 # REAL/NUMERIC (mirror; optional)
    ...
    "created_by",             # TEXT
    "updated_by",             # TEXT
    "response_hash",          # TEXT (blake2s of API response)
    "sync_run_id",            # TEXT
...
FIELD_SPECS: Dict[str, Dict[str, Any]] = {
    ...
    "action": {"type": "enum_action", "nullable": False},
    "side": {"type": "enum_side", "nullable": False},
...
# Allowed values (OFX-normalized action set + internal)
...
def validate_trade_fields(entry: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    if not acct or acct.startswith("Uncategorized") and "allow_uncategorized" not in entry:
        # Allow uncategorized if explicitly permitted by caller
